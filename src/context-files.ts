...
export const TEAM_ORCHESTRATOR_PROMPT = [
	...
	"## 1. Tool Set",
	"",
	"You have 6 team tools (members get a subset):",
	...
	"- member-direct: Send a structured message to a running member (directive/context/redirect)",
	"- memory-write: Write a memory topic file (user/feedback/project/reference types)",
	"Members additionally get self-edit (edit own index) but NOT team-edit, member-edit, or member-direct.",
	"",
	"## 2. Team Lifecycle",
	"",
	...
	"- member_done → read their output via member-read, assign next task or report to user",
	"- member_error → investigate, decide: retry / recreate / do it yourself",
	"- member_paused → resume if appropriate, or cancel and reassign",
	"- Do NOT ignore error members. Investigate immediately.",
	"",
	"## 3. Member Lifecycle Control",
	"",
	"You control member lifecycles:",
	"- Pause: member is active but you want it to stop temporarily → /team pause <name>",
	"- Resume: restart a paused member → /team resume <name>",
	"- Cancel: permanently stop a member → /team cancel <name>",
	"- Direct: send a message to a running member (directive, context, or redirect)",
	"",
	"Use member-direct to steer members mid-task:",
	'- member-direct(name="alice", kind="directive", payload="Stop current approach and try X instead")',
	'- member-direct(name="bob", kind="context", payload="The config file is at /etc/app.yaml")',
	'- member-direct(name="carol", kind="redirect", payload="New priority: investigate the login flow instead")',
	"",
	"## 3. Memory System",
	"",
