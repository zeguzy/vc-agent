...
const UserMessageView = memo(function UserMessageView({ message, index }: { message: Message, index: number }) {
	return (
		<box
			borderStyle="rounded"
...
function AssistantMessageView({ message, thinkingCollapsed }: { message: Message, thinkingCollapsed?: boolean }) {
...
const ToolMessageView = memo(function ToolMessageView({ message }: { message: Message }) {
	...
	const icon = message.toolStatus === "running" ? icons.toolRunning
		...
		: icons.toolDone
	const statusFg = message.toolStatus === "running" ? colors.textMuted
		: message.toolStatus === "error" ? colors.error
		: colors.success

	const borderColor = message.toolStatus === "running" ? colors.borderActive
		: message.toolStatus === "error" ? colors.error
		: colors.borderSoft

	const { label, lines } = formatToolDetail(message.toolName, message.toolArgs)
	const resultLines = message.toolName !== "read" && (message.toolStatus === "done" || message.toolStatus === "error")
	...
	return (
		...
		>
			...
			{lines.length > 0 && (
				<box flexDirection="column" paddingLeft={3} paddingRight={1} paddingBottom={0}>
					{lines.map((line, i) => (
						<text
							key={i}
							fg={line.startsWith("-") ? colors.error : line.startsWith("+") ? colors.success : colors.textSubtle}
						>
							{line}
						</text>
					))}
			...
			{resultLines.length > 0 && (
				<box flexDirection="column" paddingLeft={3} paddingRight={1} paddingBottom={0} marginTop={0}>
...
const ReadGroupView = memo(function ReadGroupView({ reads }: { reads: Message[] }) {
	...
	const borderColor = allRunning ? colors.borderActive
		: hasError ? colors.error
		: colors.borderSoft

	const headerIcon = allRunning ? icons.toolRunning
		: hasError ? icons.toolError
		: icons.toolDone
	const headerFg = allRunning ? colors.textMuted
		: hasError ? colors.error
		: colors.success
	...
	return (
		...
		>
			...
			{reads.map((msg, i) => {
				...
				const statusIcon = msg.toolStatus === "running" ? "○"
					: msg.toolStatus === "error" ? "✗"
					: "✓"
				const statusFg = msg.toolStatus === "running" ? colors.textMuted
					: msg.toolStatus === "error" ? colors.error
					: colors.success
				return (
					<box key={msg.id} flexDirection="row" paddingLeft={2} paddingRight={1}>
						...
						<text fg={colors.textSubtle}>{info.shortPath}{info.range ?? ""}</text>
					</box>
				)
			})}
		</box>
...
export function getReadEntries(messages: Message[]): ReadEntry[] {
	return messages
		.filter((m) => m.role === "tool" && m.toolName === "read")
		.map(extractReadInfo)
}

export function MessageList({ messages, scrollRef, thinkingCollapsed }: { messages: Message[], scrollRef?: { current: ScrollBoxRenderable | null }, thinkingCollapsed?: boolean }) {
		...
		>
			<box flexDirection="column" paddingLeft={2} paddingRight={2} paddingBottom={1}>
				...
				{items.map((item, i) => {
					...
					if (msg.role === "user") return <UserMessageView key={msg.id} message={msg} index={item.index} />
					if (msg.role === "tool") return <ToolMessageView key={msg.id} message={msg} />
					return <AssistantMessageView key={msg.id} message={msg} thinkingCollapsed={thinkingCollapsed} />
				})}
			</box>
		</scrollbox>
	)
