...
export function StatusBar({ model, mode, contextPercent, contextTokens, contextWindow, contextDisplay }: StatusBarProps) {
	const modeColor =
		mode === "insert" ? colors.success
		: colors.primary

	const ctxColor =
		contextPercent === null ? colors.textMuted
		: contextPercent < 50 ? colors.success
		: contextPercent < 80 ? colors.warning
		: colors.error
		...
		>
			...
			{hasContext && contextDisplay === "full" && (
				<>
					...
					{contextPercent !== null && (
						<text fg={ctxColor}> ({contextPercent.toFixed(0)}%)</text>
					)}
				</>
			)}
		</box>
