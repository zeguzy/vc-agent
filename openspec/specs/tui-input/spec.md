...
#### Scenario: 模式快捷键提示
- **WHEN** 渲染状态行右侧提示
- **THEN** INSERT 模式显示 `Enter to send · Shift+Enter newline · Esc: normal`（或运行时 `Enter to queue · Esc: normal`），NORMAL 模式显示 `i: type · j/k: scroll · t: thinking`
...
#### Scenario: 输入框始终可用
- **WHEN** Agent 正在运行
- **THEN** Textarea SHALL 保持可用（可输入、可提交），placeholder 显示 `Queue a message…`，右侧提示显示 `Enter to queue · Esc: normal`
